...
class Backtest(object):
    ...
    def _generate_trading_instances(self):
        ...
        # elif self.data_handler_cls.__name__ == 'IBKRDataHandler':
        #     continue
        if self.data_handler_cls.__name__ == "IBKRDataHandler":
        ...
        elif self.data_handler_cls.__name__ == 'HistoricCSVDataHandler':
            self.data_handler = self.data_handler_cls(
                                                    self.events, 
                                                    self.data_dir, 
                                                    self.symbol_list
                ...
                try:
                    ...
                    if event is not None :
                        if isinstance(event, MarketEvent):
                            ...
                            # print("\n", "$" * 10, "\n", "\nisinstance SignalEvent")
                            # print(">>>vars(event)", vars(event))
                            self.signals += 1
                            self.portfolio.update_signal(event)

                        elif isinstance(event, OrderEvent):
                            print("\nisinstance OrderEvent")
                            print(">>>vars(event)", vars(event))
                            self.orders += 1
                            self.execution_handler.execute_order(event)

                        elif isinstance(event, FillEvent):
                            print("\nisinstance FillEvent")
                            print(">>>vars(event)", vars(event))
                            self.fills += 1
    ...
    def simulate_trading(self):
        ...
        # Join the ibkr_thread once backtesting is finished
