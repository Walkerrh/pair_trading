...
class Portfolio(object):
    ...
    def update_signal(self, event):
        # print(">>update_signal", vars(event))
        """
    ...
    def generate_naive_order(self, signal):
        # print(">>generate_naive_order", vars(signal))
        ...
        signalevent_type = signal.type
        signalevent_time = signal.datetime
        direction = signal.signal_type
        strength = signal.strength

        mkt_quantity = floor(50 * strength)
        current_quantity = self.current_positions[symbol]
        # print(">current_quantity", current_quantity)
        order_type = "MKT"

        current_price = self.bars.get_latest_bar_value(symbol=symbol, value_type="Close")
        # print(">current_price", current_price)

        shares_to_buy = 15000 / current_price # dollar amount per security / security price
    ...
    def update_positions_after_fill(self, fill):
        # print(">>update_positions_after_fill", vars(fill))
        """
    ...
    def update_holdings_after_fill(self, fill):
        # print(">>update_holdings_after_fill", vars(fill))
        """
    ...
    def output_summary_stats(self):
        ...
        390 trading minutes in a day
        98280 trading minutes in a year
        """
        # sharpe_ratio = create_sharpe_ratio(returns, periods=252) 
        # sharpe_ratio = create_sharpe_ratio(returns, periods=390) 
        sharpe_ratio = create_sharpe_ratio(returns, periods=98280) # trading by the minute so we measure by the minute
        ...
        self.equity_curve.to_csv("/Users/walkerhutchinson/Desktop/CODE/z_CPED/equity.csv")
        return stats